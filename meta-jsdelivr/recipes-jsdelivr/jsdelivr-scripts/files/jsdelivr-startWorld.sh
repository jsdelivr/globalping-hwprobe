...
while [ 1 ]; do
    ...
    if [ "$RUNNING" != "true" ]; then
        /usr/bin/docker run -d  --env GP_HOST_HW --env GP_HOST_DEVICE --env GP_HOST_FIRMWARE --env GP_HW_UID --log-driver local --log-opt max-size=10m --network host --restart=always --name globalping-probe globalping-probe
    fi
